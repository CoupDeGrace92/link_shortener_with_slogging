...
func Test_requestLogger(t *testing.T) {
	...
	if logBuffer.String() != expectedLogString {
		t.Errorf("Expected log not equal to actual log.\n Expected: %s\n Actual: %s\n", expectedLogString, logBuffer.String())
	}
	if rr.Code != expectedStatusCode {
		t.Errorf("Expected status code not equal to actual status code:\n Expected: %v\n Actual: %v\n", expectedStatusCode, rr.Code)
	}
}
